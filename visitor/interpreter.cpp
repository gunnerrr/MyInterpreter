...
void visitor::Interpreter::visit(parser::ASTSqrtNode *sqrt){
    // Visit expression node to update current value/type
    sqrt -> expr -> accept(this);

    // Print, depending on type
    switch(current_expression_type){
        case parser::INT:
        if(current_expression_value.i>=0) {
            auto number_i=pow(current_expression_value.i,0.5);
            std::cout<<number_i;
        }
        else {
            std::cout<<"Your number must be positive!";
            break;
        }
            break;
        case parser::REAL:
        if(current_expression_value.r>=0) {
            auto number_r=pow(current_expression_value.r,0.5);
            std::cout<<number_r;
        }
        else {
            std::cout<<"Your number must be positive!";
...
void visitor::Interpreter::visit(parser::ASTFactNode *fact){
    // Visit expression node to update current value/type
    ...
    for(auto i=1;i<=current_expression_value.i;i++) {
        res*=i;
    }
        std::cout<<res;
    }
...
void visitor::Interpreter::visit(parser::ASTSinNode *sinus){
    // Visit expression node to update current value/type
    sinus -> expr -> accept(this);

    if(current_expression_type==parser::INT) {
            auto number_i=sin(current_expression_value.i);
            std::cout<<number_i;
           }
    else if(current_expression_type==parser::REAL) {
            auto number_r=sin(current_expression_value.r);
            std::cout<<number_r;
           }
}

void visitor::Interpreter::visit(parser::ASTCosNode *cosinus){
    // Visit expression node to update current value/type
    cosinus -> expr -> accept(this);

    // Print, depending on type
    if(current_expression_type==parser::INT) {
            auto number_i=cos(current_expression_value.i);
            std::cout<<number_i;
           }
    else if(current_expression_type==parser::REAL) {
            auto number_r=cos(current_expression_value.r);
            std::cout<<number_r;
           }
}

void visitor::Interpreter::visit(parser::ASTTangNode *tang){
    // Visit expression node to update current value/type
    tang -> expr -> accept(this);

    if(current_expression_type==parser::INT) {
            auto number_i=tan(current_expression_value.i);
            std::cout<<number_i;
           }
    else if(current_expression_type==parser::REAL) {
            auto number_r=tan(current_expression_value.r);
            std::cout<<number_r;
           }
}
void visitor::Interpreter::visit(parser::ASTLogNode *logar){
    // Visit expression node to update current value/type
    logar -> expr -> accept(this);
    ...
    if(current_expression_type==parser::INT) {
        if(current_expression_value.i<=0) {
            ...
            auto number_i=log(current_expression_value.i);
            std::cout<<number_i;
           }
    else if(current_expression_type==parser::REAL) {
        if(current_expression_value.r<=0) {
            ...
            auto number_r=log(current_expression_value.r);
            std::cout<<number_r;
           }
}
