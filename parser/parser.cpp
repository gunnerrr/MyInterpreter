...
ASTStatementNode* Parser::parse_statement() {
    ...
    switch(current_token.type){
        ...
        case lexer::TOK_SQRT:
        return parse_sqrt_statement();

        case lexer::TOK_FACTORIAL:
        return parse_fact_statement();

        case lexer::TOK_SIN:
        return parse_sin_statement();

        case lexer::TOK_COS:
        return parse_cos_statement();

        case lexer::TOK_TANG:
        return parse_tang_statement();
...
ASTSqrtNode* Parser::parse_sqrt_statement() {
    ...
    // Make sure it's a ';'
    if(current_token.type != lexer::TOK_SEMICOLON)
        throw std::runtime_error("Expected ';' after sqrt statement on line "
                                 + std::to_string(current_token.line_number) + ".");

    // Return return node
    return new ASTSqrtNode(expr, line_number);
}

ASTFactNode* Parser::parse_fact_statement() {
    ...
    // Make sure it's a ';'
    if(current_token.type != lexer::TOK_SEMICOLON)
        throw std::runtime_error("Expected ';' after fact statement on line "
                                 + std::to_string(current_token.line_number) + ".");

    // Return return node
    return new ASTFactNode(expr, line_number);
}
ASTSinNode* Parser::parse_sin_statement() {
    ...
    // Make sure it's a ';'
    if(current_token.type != lexer::TOK_SEMICOLON)
        throw std::runtime_error("Expected ';' after sin statement on line "
                                 + std::to_string(current_token.line_number) + ".");

    // Return return node
    return new ASTSinNode(expr, line_number);
}

ASTCosNode* Parser::parse_cos_statement() {
    ...
    // Make sure it's a ';'
    if(current_token.type != lexer::TOK_SEMICOLON)
        throw std::runtime_error("Expected ';' after cos statement on line "
                                 + std::to_string(current_token.line_number) + ".");

    // Return return node
    return new ASTCosNode(expr, line_number);
}

ASTTangNode* Parser::parse_tang_statement() {
    ...
    // Make sure it's a ';'
    if(current_token.type != lexer::TOK_SEMICOLON)
        throw std::runtime_error("Expected ';' after tang statement on line "
                                 + std::to_string(current_token.line_number) + ".");

    // Return return node
    return new ASTTangNode(expr, line_number);
}

ASTLogNode* Parser::parse_log_statement() {
    ...
    // Make sure it's a ';'
    if(current_token.type != lexer::TOK_SEMICOLON)
        throw std::runtime_error("Expected ';' after log statement on line "
                                 + std::to_string(current_token.line_number) + ".");

    // Return return node
    return new ASTLogNode(expr, line_number);
}
